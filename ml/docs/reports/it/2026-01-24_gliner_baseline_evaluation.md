# Valutazione Baseline GLiNER-PII

**Data:** 2026-01-24
**Modello:** knowledgator/gliner-pii-base-v1.0
**Soglia:** 0.3 (bilanciata)
**Test:** 35
**Tempo:** 3.8s
...
### Confronto con legal_ner_v2

| Modello | F1 Strict | Precisione | Richiamo | Tasso Superamento |
|---------|-----------|-----------|--------|-----------|
| **GLiNER-PII-base** | **0.325** | 0.287 | 0.373 | 11.4% |
| legal_ner_v2 | 0.788 | - | - | 60.0% |
| **Differenza** | **-0.463** | - | - | -48.6pp |

### Conteggi SemEval

| Metrica | Valore | Descrizione |
|---------|-------|-------------|
| COR | 25 | Corretto (limite + tipo) |
| INC | 4 | Limite OK, tipo errato |
| PAR | 24 | Corrispondenza parziale |
| MIS | 14 | Mancato (FN) |
| SPU | 34 | Spurious (FP) |

---

## 2. Risultati per Categoria

| Categoria | Strict | COR | INC | PAR | MIS | SPU |
|-----------|--------|-----|-----|-----|-----|-----|
...
### Punti di Forza GLiNER

- Zero-shot: Non richiede addestramento per nuovi tipi di entità
- Multilingue: Supporto nativo per lo spagnolo
- Etichette flessibili: Specificate a runtime

### Debolezze Osservate

- Formati spagnoli specifici (DNI, NIE, IBAN con spazi)
- Date testuali in spagnolo (XV de marzo de MMXXIV)
- Identificatori legali spagnoli (ECLI, riferimenti catastali)
- Contesto negativo (NO tener DNI)

### Raccomandazione

legal_ner_v2 supera GLiNER zero-shot (0.325 F1). Il fine-tuning specifico per il dominio legale spagnolo aggiunge valore.

> **Nota successiva (2026-02-04):** Il fine-tuning LoRA di Legal-XLM-RoBERTa-base è stato testato: F1 0.016 avversario (grave overfitting). Scartato. Il modello base della pipeline rimane RoBERTa-BNE CAPITEL NER (`legal_ner_v2`).
...
```python
# Etichette usate (mappatura a tipi ContextSafe)
...
# Soglia
threshold = 0.3  # Bilanciata (consigliata per produzione)
```

---

**Generato da:** `scripts/evaluate/evaluate_gliner_baseline.py`
**Data:** 2026-01-24
